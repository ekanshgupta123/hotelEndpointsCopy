        ...
        const getRooms = async () => {
            ...
            try {
                ...
                if (hotelsData.length > 0) {
                    ...
                    hotelsData[0].rates.forEach(async (rate: { serp_filters: any, rg_ext: any, room_data_trans: { main_name: string; misc_room_type: string; main_room_type: any; bathroom: any; bedding_type: any; }; payment_options: { payment_types: { tax_data: { taxes: any[] }, cancellation_penalties: { policies: any[]; }; }[]; }; daily_prices: string[]; amenities_data: any; meal: string; room_name: any; }, index) => {
                    ...
                    // console.log(roomGroups, 'room data');
                    setStuff(roomGroups);
                        ...
                        {toggleStates[index] && 
                            <div>
                                ...
                                {value.options.map((option, idx) => (
                                    <div>
                                        <div key={idx} style={{ display: 'flex', border: '1px solid black', padding: '0.5%', marginBottom: '10px', marginTop: '1%' }}>
                                            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginRight: '1.5%' }}>
                                                ...
                                                <p>{handleDots(imgMap[`${index}-${idx}`] || 0, groups[index].length)}</p>
                                            </div>
                                            ...
                                            <div style={{ width: '45%' }}>
                                                {option.cancellation.includes('\n') && option.cancellation.split('\n').map((opt, index) => <p style={{ whiteSpace: 'pre-wrap' }}>{index == 1 && ('↳' + opt) || opt }</p>)
                                                || (<p>{option.cancellation}</p>)}
                                            </div>
                                            <div style={{ display: 'flex', flexDirection: 'column' }}>
                                                <p style={{ width: '10%' }}>${option.price}</p>
                                                {option.taxes.map(tax => <label style={{ fontSize: '12px', width: '85%' }}>+ ${tax.amount} as {tax.name.replace('_', ' ')}</label>)}
