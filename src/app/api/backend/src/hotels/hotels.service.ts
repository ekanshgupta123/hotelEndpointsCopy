...
export class HotelsService {
  ...
  async staticImages(ID: string, body: any) {
    ...
    return (
      (hotel[0].room_groups.filter(
        (rate) => roomMatch(rate.rg_ext, body) == true,
      ).length != 0 &&
        hotel[0].room_groups.filter(
          (rate) => roomMatch(rate.rg_ext, body) == true,
        )[0].images) || ['https://placehold.jp/{size}.png']
    );
  }

  async redisCache(keys: string[], page: number, flag: boolean, regID: number, data?: any[]) {
    try {
      ...
      if (flag) {
        ...
        return { data: resultArr, total: await this.cacheService.get(`total-${regID}`) };
      } else {
        resultArr = await Promise.all(
          keys.map(async (key, index) => {
            const toPass =
              index == 1
                ...
                : data;
            await this.utilService.dataPipeline(key, toPass, 30);
            await this.cacheService.set(`total-${regID}`, data.length, 30000);
            return await this.utilService.getPaginatedData(key, end - 24, end);
          }),
        );
      }
  ...
  async fetchHotelDetails(hotelId: string, language: string): Promise<any> {
    ...
    return this.httpService
      ...
      .pipe(
        ...
        catchError((error) => {
          console.error(
            `Error fetching hotel details for ID ${hotelId}:`,
  ...
  async fetchDetailsForMultipleHotels(params: {
    checkin: string;
    checkout: string;
    residency: string;
    language: string;
    ...
    return this.httpService
      ...
      .pipe(
        ...
        catchError((error) => {
          console.error(
            'Error in fetchDetailsForMultipleHotels:',
            error.response?.data || error.message,
          );
          return throwError(
            new HttpException(
  ...
  async searchHotels(searchParams: any): Promise<any> {
    ...
    const responseArr = await this.httpService
      .post(
